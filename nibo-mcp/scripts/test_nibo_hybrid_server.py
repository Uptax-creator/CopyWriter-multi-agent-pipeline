#!/usr/bin/env python3
"""
Script para testar servidor híbrido Nibo MCP
"""
...
class NiboHybridServerTester:
    """Classe para testar servidor híbrido Nibo"""
    ...
    def start_stdio_server(self) -> subprocess.Popen:
        ...
        print("🚀 Iniciando servidor Nibo STDIO...")
        
        process = subprocess.Popen(
            [sys.executable, "/Users/kleberdossantosribeiro/omie-mcp/nibo-mcp/nibo_mcp_server_hybrid.py", "--mode", "stdio"],
    ...
    def start_http_server(self) -> subprocess.Popen:
        ...
        print("🚀 Iniciando servidor Nibo HTTP...")
        
        process = subprocess.Popen(
            [sys.executable, "/Users/kleberdossantosribeiro/omie-mcp/nibo-mcp/nibo_mcp_server_hybrid.py", "--mode", "http", "--port", "3002"],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            text=True
        )
    ...
    def test_stdio_server(self, process: subprocess.Popen) -> dict:
        ...
        # Teste 4: Tool Call (consultar_socios - específico do Nibo)
        print("  📋 Teste 4: Tool Call (consultar_socios)...")
        socios_request = {
            "jsonrpc": "2.0",
            "id": "test-socios",
            "method": "tools/call",
        ...
        try:
            request_json = json.dumps(socios_request) + "\n"
            process.stdin.write(request_json)
            process.stdin.flush()
            ...
            if response_line:
                ...
                print(f"    {'✅' if test_passed else '❌'} Consultar Sócios")
                if not test_passed:
                    results["success"] = False
            else:
                results["tests"].append({
                ...
                results["success"] = False
                print("    ❌ Consultar Sócios - No response")
        except Exception as e:
            results["tests"].append({
                "name": "consultar_socios",
                "passed": False,
            ...
            results["success"] = False
            print(f"    ❌ Consultar Sócios - Error: {e}")
        
        return results
    
    def test_http_server(self) -> dict:
        """Testa servidor HTTP"""
        print("\n🧪 Testando servidor Nibo HTTP...")
        ...
        # Teste 4: Tool Call (testar_conexao)
        print("  📋 Teste 4: Tool Call (testar_conexao)...")
        try:
            payload = {"arguments": {}}
        ...
        # Teste 5: Tool Call (consultar_socios - específico do Nibo)
        print("  📋 Teste 5: Tool Call (consultar_socios)...")
        try:
            payload = {"arguments": {}}
            response = requests.post(f"{base_url}/mcp/tools/consultar_socios", json=payload, timeout=5)
            test_passed = response.status_code == 200
            results["tests"].append({
                "name": "mcp_socios_call",
                "passed": test_passed,
                "status_code": response.status_code,
                "response": response.json() if test_passed else None
            })
    ...
    def run_tests(self):
        ...
        print("🧪 Iniciando testes do servidor híbrido Nibo...")
        print("=" * 60)
        ...
        # Relatório final
        print("\n" + "=" * 60)
        print("📊 RELATÓRIO FINAL - SERVIDOR HÍBRIDO NIBO")
        print("=" * 60)
        ...
        # Salvar relatório
        report_file = f"/Users/kleberdossantosribeiro/omie-mcp/nibo-mcp/nibo_hybrid_test_report_{int(time.time())}.json"
...
def main():
    ...
    try:
        ...
        if all_passed:
            print("\n🎉 Todos os testes passaram! O servidor híbrido Nibo está funcionando corretamente.")
            print("\n📋 Próximos passos:")
            print("1. Configure Claude Desktop para usar: python nibo_mcp_server_hybrid.py --mode stdio")
            print("2. Use integrações web com: python nibo_mcp_server_hybrid.py --mode http --port 3002")
            print("3. Ferramentas exclusivas do Nibo: consultar_socios, incluir_socio, incluir_multiplos_clientes")
            sys.exit(0)
        else:
